...
--=========================================================
-- ★ KEY SYSTEM (맨 위 — 통과 못 하면 스크립트 종료)
--=========================================================
...
if not keyPassed then
    -- 키 입력 UI 띄우고 통과할 때까지 대기
    local keyGui = Instance.new("ScreenGui")
    keyGui.Name = "RATKeyGui"
    ...
    kFrame.Size = UDim2.fromOffset(420, 240)
    kFrame.Position = UDim2.fromScale(0.5, 0.5)
    ...
    kStroke.Color = Color3.fromRGB(220, 70, 70)
    kStroke.Thickness = 2
    ...
    kTitle.Text = "🔒 RAT HUB"
    kTitle.Font = Enum.Font.FredokaOne
    kTitle.TextSize = 32
    ...
    kSub.Size = UDim2.new(1, 0, 0, 24)
    kSub.Position = UDim2.new(0, 0, 0, 62)
    kSub.BackgroundTransparency = 1
    kSub.Text = "라이선스 키를 입력하세요"
    kSub.Font = Enum.Font.GothamMedium
    kSub.TextSize = 14
    kSub.TextColor3 = Color3.fromRGB(150, 150, 165)
    kSub.ZIndex = 51
    kSub.Parent = kFrame

    local kInput = Instance.new("TextBox")
    kInput.Size = UDim2.new(1, -60, 0, 44)
    kInput.Position = UDim2.new(0, 30, 0, 100)
    kInput.BackgroundColor3 = Color3.fromRGB(30, 30, 38)
    kInput.BorderSizePixel = 0
    kInput.Text = ""
    kInput.PlaceholderText = "RAT-XXXX-XXXX-XXXX"
    kInput.Font = Enum.Font.GothamBold
    kInput.TextSize = 15
    kInput.TextColor3 = Color3.fromRGB(240, 240, 240)
    kInput.PlaceholderColor3 = Color3.fromRGB(100, 100, 115)
    ...
    kStatus.Size = UDim2.new(1, -60, 0, 20)
    kStatus.Position = UDim2.new(0, 30, 0, 148)
    kStatus.BackgroundTransparency = 1
    kStatus.Text = ""
    kStatus.Font = Enum.Font.GothamMedium
    kStatus.TextSize = 13
    kStatus.TextColor3 = Color3.fromRGB(255, 130, 130)
    kStatus.TextXAlignment = Enum.TextXAlignment.Left
    kStatus.ZIndex = 51
    kStatus.Parent = kFrame

    local kSubmit = Instance.new("TextButton")
    kSubmit.Size = UDim2.new(1, -60, 0, 44)
    kSubmit.Position = UDim2.new(0, 30, 1, -60)
    kSubmit.BackgroundColor3 = Color3.fromRGB(220, 70, 70)
    kSubmit.Text = "인증"
    ...
    Instance.new("UICorner", kSubmit).CornerRadius = UDim.new(0, 10)

    local passedSignal = false

    local function trySubmit()
        local k = (kInput.Text or ""):upper():gsub("%s", "")
        if k == "" then
            kStatus.Text = "키를 입력하세요"
            return
        end
        if isValid(k) then
            kStatus.TextColor3 = Color3.fromRGB(120, 255, 120)
            kStatus.Text = "인증 성공! 로딩중..."
            pcall(function() if writefile then writefile("rathub_key.txt", k) end end)
            passedSignal = true
        else
            kStatus.TextColor3 = Color3.fromRGB(255, 130, 130)
            kStatus.Text = "유효하지 않은 키입니다"
        end
    end
