--[[
    RAT HUB v17 — Loader (no key system)
    사용법:
...
local function fetchSource(url)
    ...
    if not ok then
        return nil, "HTTP 요청 실패: " .. tostring(res)
    end
    if not res or res == "" then
        return nil, "빈 응답 — URL 확인: " .. url
    end
    if res:sub(1, 3) == "404" then
...
local function boot()
    ...
    if not fn then
        warn("[RAT HUB] 컴파일 실패: " .. tostring(compileErr))
        return
    end

    local ok, runErr = pcall(fn)
    if not ok then
        warn("[RAT HUB] 실행 오류: " .. tostring(runErr))
    end
end
